...
//AUTOGENERATED FROM ../src/Robbable.java
...
//AUTOGENERATED FROM ../src/HalloweenNight.java
class HalloweenNight {
    private static final int TRICK_OR_TREATER_SIZE = 5;

    private TrickOrTreater[] cryptKickerFive = new TrickOrTreater[TRICK_OR_TREATER_SIZE];
    private TrickOrTreater[] ghoulGang = new TrickOrTreater[TRICK_OR_TREATER_SIZE];
    ...
    public String toString() {
        String cryptString = "cryptKickerFive: ";

        for (int i = 0; i < cryptKickerFive.length - 1; i++) {
            cryptString += cryptKickerFive[i].toString() + ", ";
        }

        cryptString += cryptKickerFive[cryptKickerFive.length - 1];

        String ghoulString = "ghoulGang: ";

        for (int i = 0; i < ghoulGang.length - 1; i++) {
            ghoulString += ghoulGang[i].toString() + ", ";
        }

        ghoulString += ghoulGang[ghoulGang.length - 1];

        return cryptString + " versus " + ghoulString;
    }
    ...
    public void compareTeams() {
        ...
        if (ghoulCandy > cryptCandy) {
            System.out.println("ghoulGang is favored.");
        } else if (cryptCandy > ghoulCandy) {
    ...
    private int getCandyOfTeam(TrickOrTreater[] team) {
        int candy = 0;
        
        for (int i = 0; i < team.length; i++) {
            candy += team[i].getNumCandy();
        }

        return candy;
    }

    private void trickOrTreatAll(TrickOrTreater[] team) {
        for (TrickOrTreater t : team) {
            t.trickOrTreat();
        }
    }
    ...
    public void battle(int winningCandyThreshold) {
        ...
        while (getCandyOfTeam(cryptKickerFive) < winningCandyThreshold 
            && getCandyOfTeam(ghoulGang) < winningCandyThreshold) {
            if (currentTurn == 1) {
                trickOrTreatAll(cryptKickerFive);
            } else {
                trickOrTreatAll(ghoulGang);
            }
        ...
        if (getCandyOfTeam(cryptKickerFive) >= winningCandyThreshold) {
            System.out.println("cryptKickerFive won!");
        } else if (getCandyOfTeam(ghoulGang) >= winningCandyThreshold) {
...
public abstract class TrickOrTreater implements Comparable<TrickOrTreater> {
    ...
    protected TrickOrTreater(String name, int age, int numCandy) {
        ...
        this.age = age;
        this.numCandy = Math.max(0, numCandy);
    }
    ...
    protected void loseCandy(int amount) {
        if (amount > 0) {
            numCandy = Math.max(numCandy - amount, 0);
        }
    }
    ...
    public int compareTo(TrickOrTreater other) {
        // greater than other if it has more candy

        if (this.numCandy == other.numCandy) {
            return this.age - other.age;
        }

        return this.numCandy - other.numCandy;
    }

}
//AUTOGENERATED FROM ../src/Ghost.java
class Ghost extends TrickOrTreater {
    private int robberiesConducted;
    

    public Ghost(String name, int age, int numCandy, int robberiesConducted) {
        super(name, age, numCandy);

        this.robberiesConducted = robberiesConducted;
    }
    ...
    public void rob(TrickOrTreater toRob) {
        // Check if robbable
        if (toRob instanceof Robbable) {
            int candyRobbed = ((Robbable) toRob).beRobbed();
            
            numCandy += candyRobbed;
    ...
    @Override
    public String toString() {
        return String.format("%s/%d/%d/%d", name, age, numCandy, robberiesConducted);
    }
    
    @Override
    public int compareTo(TrickOrTreater other) {
        // assume numm reference will not be passed in
        if (other.getClass() == this.getClass()) {
            ...
            if (superCompare == 0) {
                return this.robberiesConducted - otherGhost.robberiesConducted;
            }
...
//AUTOGENERATED FROM ../src/Witch.java
class Witch extends TrickOrTreater implements Robbable {
    ...
    public Witch(String name, int age, int numCandy, String signatureCackle) {
        ...
        if (signatureCackle == null || signatureCackle.trim().length() == 0) {
    ...
    @Override
    public int beRobbed() {
        int piecesToBeRobbed = Math.min(numCandy, 6);

        numCandy -= piecesToBeRobbed;
    ...
    public int compareTo(TrickOrTreater other) {
        if (other.getClass() == this.getClass()) {
            ...
            if (superCompare == 0) {
                return this.signatureCackle.length() - otherWitch.signatureCackle.length();
